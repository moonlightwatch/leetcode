...
func bestRotation(nums []int) int {
	// 执行用时：96 ms, 在所有 Go 提交中击败了66.67% 的用户
	// 内存消耗：9.2 MB, 在所有 Go 提交中击败了91.67% 的用户

	// 思路：主要是避免嵌套循环导致的超时。所以将计算过程分为几次循环统计操作：
	//      分析1：因为每次轮调，第一个数调到最后，必定会变成记分项。而其他的位置，轮调后可能会变成不记分。
	//      分析2：从计分变为不记分的情况只有一种，即 nums[i]==i， 这种情况在下次轮调中，会变为不记分。
	//      分析3：我们可以通过计算，获知在几次轮调后，会处于nums[i]==i的状态。据此，只要知道未轮调的得分数量，即可逐次计算得分：
